...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,),name="" ):
        ...
        inputs=InputLayer(input_shape=shape,name=name)
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        """
         ...
         :return: None
         """
        self.model.add(Dense(num_nodes,activation=activation,name=name,trainable=trainable))
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        self.model.add(Conv2D(num_of_filters,kernel_size=kernel_size,padding=padding,strides=strides,activation=activation,name=name,trainable=trainable))
        #self.trainable=trainable
        return layer
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        ...
        self.model.add(MaxPooling2D(pool_size=pool_size,padding=padding,strides=strides,name=name))
        return layer
    ...
    def get_weights_without_biases(self,layer_number=None,layer_name=""):
        ...
        # elif layer_name is not None:
        #             for layer in self.model.layers:
        #                 if layer.name==layer_name and self.model.get_layer(name=layer_name).count_params()!=0:
        #                     return layer.weights[0]
        # return None     
        elif layer_name != "":
            try:
    ...
    def get_biases(self,layer_number=None,layer_name=""):
        ...
        # if layer_number is not None:
        #     if layer_number!= 0:
        #         return self.model.layers[layer_number-1].weights[1]
        #     else:
        #         return None
        # elif layer_name is not None:
        #         for layer in self.model.layers:
    ...
    def set_weights_without_biases(self,weights,layer_number=None,layer_name=""):
        ...
        try:
            if layer_number is not None:
                if layer_number!=0:
                    self.model.layers[layer_number-1].weights=weights
                else:
                    return None
            elif layer_name is not None:
                    for layer in self.model.layers:
        ...
        return none'''
        if layer_number is not None:
           bias=self.get_biases(layer_number,layer_name)
           weight=[weights,bias]
           self.model.get_layer(index=layer_number-1,name=layer_name).set_weights(weight)
        else:
           bias=self.get_biases(layer_number,layer_name)
           weight=[weights,bias]
           self.model.get_layer(name=layer_name).set_weights(weight)
    ...
    def evaluate(self,X,y):
        ...
        loss,metric=self.model.evaluate(X,y,verbose=2)
        #print("M",metric)
        return (loss,metric)
    ...
    def train(self, X_train, y_train, batch_size, num_epochs):
        """
         ...
         """
        self.model.compile(optimizer=self.optimizer, loss=self.loss, metrics=self.metric)
        result=self.model.fit(X_train, y_train, batch_size,num_epochs)
